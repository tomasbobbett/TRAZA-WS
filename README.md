...
La estética combina superficies claras, verde profundo, tipografía sans serif y fotografías grandes con esquinas suaves. La portada presenta texto y fotografía en dos columnas, apiladas en móvil; el catálogo conserva imágenes amplias. La sección de consulta combina una fotografía ilustrativa con tres pasos sobre un panel verde. La ubicación incluye un mapa interactivo de Google Maps con el código proporcionado por el cliente, los datos de contacto y un enlace independiente para abrir las indicaciones. El mapa se carga al acercarse a la sección.

El vivero suaviza la rueda del mouse, el trackpad y los gestos táctiles con [Lenis 1.3.26](https://github.com/darkroomengineering/lenis), servido localmente desde `webs/assets/vendor/lenis-1.3.26/` con su licencia MIT. Usa una desaceleración perceptible (`lerp: 0.065`) e inercia al soltar el dedo (`syncTouch: true`). Conserva el desplazamiento nativo al usar el teclado o seguir un enlace interno. La inercia se detiene al abrir el menú o la revisión de la consulta; los campos y el mensaje mantienen su scroll propio. La preferencia de movimiento reducido desactiva el suavizado, también si cambia durante la visita. La navegación usa un fondo opaco para evitar recalcular el desenfoque al desplazar la página.

`node scripts/verify-vivero-scroll.mjs` comprueba la continuidad de la rueda, navegación por teclado y enlaces, scroll interno del formulario, bloqueo del menú y diálogo, mapa, cambios de movimiento reducido y gestos táctiles. Usa la misma vista previa y configuración de Playwright que `verify-vivero.mjs` y guarda el resultado en `qa-vivero/scroll-report.json`.

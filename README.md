...
La demo de **Vivero la Loma**, de El Torno, Bolivia, está en `/demo/vivero/`. Está orientada a frutales injertados para productores, municipios y organizaciones. Las consultas de esta demo se dirigen al número del cliente, `+591 72139484`; las demás demos conservan el contacto de TRAZA. Incluye catálogo con filtros, selección de especies y tipo de comprador, formulario que prepara un mensaje revisable de WhatsApp, preguntas frecuentes y mapa con el código `2J5J+7W, El Torno, Bolivia`. Cada sección se ajusta al alto visible de la pantalla y conserva la composición alineada a la izquierda, con la portada grande y las tarjetas completas en escritorio. En pantallas pequeñas las tarjetas se recorren horizontalmente y las preguntas muestran una respuesta por vez. La cotización tiene cuatro pasos, conserva los datos al volver y permite revisar el mensaje antes de abrir WhatsApp.
...
Con la vista previa en marcha, `node scripts/verify-vivero.mjs` verifica Vivero la Loma en tres idiomas y once tamaños de pantalla, desde 320 × 568 hasta 1920 × 1080, incluyendo orientación horizontal. Comprueba el alto de las secciones y la alineación de la portada en cada paso del formulario, ausencia de desbordes, filtros, carruseles, preguntas, destinatario y contenido de WhatsApp, persistencia de datos al traducir, menú móvil y contenido sin JavaScript. No envía mensajes. Guarda capturas e informe en `qa-vivero/`, fuera del repositorio. Usa los mismos requisitos de Playwright, `NODE_PATH` y `PREVIEW_URL` de las comprobaciones de idiomas. Las fuentes de las fotografías están en `scripts/vivero-image-sources.json`; las tipografías locales incluyen sus licencias en `webs/assets/vivero/`.

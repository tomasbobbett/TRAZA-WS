...
try {
    for (const demo of DEMO_PAGES) {
        ...
        for (const lang of ['es', 'en', 'pt']) {
            ...
            for (const width of [1440, 1024, 390, 320]) {
                ...
                const layout = await page.evaluate(() => {
                    ...
                    const nav = [...document.querySelector('.nav').children].filter(visible).filter(el => !el.matches('.nav-links,.nav__menu'));
                    ...
                    return { lang: document.documentElement.lang, overflow: document.documentElement.scrollWidth - innerWidth, overlap, escaped, clippedText, untranslated, title: document.title, heading: document.querySelector('h1').innerText };
                });
        ...
        console.log(`Checked ${demo.route}: 3 languages × 4 widths`);
        await page.close();
    }
    const issues = results.filter(r => r.overflow > 0 || r.overlap.length || r.escaped.length || r.clippedText.length || r.untranslated.length);
